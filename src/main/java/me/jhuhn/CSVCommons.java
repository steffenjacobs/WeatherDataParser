...
public class CSVCommons {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		org.apache.commons.csv.CSVParser csvFileParser = CSVFormat.DEFAULT.parse(new FileReader(new File("data_set_original.csv")));
		...
		for (CSVRecord csvRecord : csvFileParser) {
			...
			for (int i = 0; i < splitViolation.length; i++) {
				//write file
				csvPrinter.printRecord(id, state, stop_date, stop_time, location_raw, county_name, county_fips, fine_grained_location, police_department, driver_gender, driver_age_raw, driver_age, driver_race_raw, driver_race, 
						violation_raw, splitViolation[i], search_conducted, search_type_raw, search_type, contraband_found, stop_outcome, is_arrested, officer_id, officer_gender, officer_age, officer_race, officer_rankr, out_of_state);
			}
...
//			}
//			
//			System.out.println(csvFileParser.getCurrentLineNumber());
//			System.out.println(violationCSV);
		}
